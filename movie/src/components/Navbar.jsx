  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6
        ...
        <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium max-md:text-lg
                ...
                min-md:rounded-full backdrip-blur bg-black/70 md:bg-white/10 md:border
        ...
        <div className='flex items-center gap-8' >
            <SearchIcon  className='max-md:ml-4  w-8 h-8 cursor-pointer'/>
          {
            !user ? (

              <button onClick={openSignIn} className='px-4 py-1 sm:px-7 sm:py-2 bg-primary hover:bg-primary-dull
                  transition rounded-full font-medium cursor-pointer bg-red-600'> 
                  Login
            </button>
              ...
              <UserButton >
                  <UserButton.MenuItems>
                      <UserButton.Action  label='My Bokkings' labelIcon={<TicketPlus width={15} />} onClick={() =>navigate('/my-bookings')}/>
                  </UserButton.MenuItems>
                </UserButton>
            )
          }
